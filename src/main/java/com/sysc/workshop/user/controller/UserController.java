...
public class UserController {
    ...
    ResponseEntity<ApiResponse> createUser(@RequestBody CreateUserRequest request){
        ...
        try {
            ...
            return ResponseEntity.status(201).body(new ApiResponse(user.getName() +" successfully created!",user ));
    ...
    @PutMapping("/")
    ResponseEntity<ApiResponse> updateUser(@RequestBody UpdateUserRequest  request, @PathVariable UUID userID){
        try {
            UserDto user = userService.updateUserById(request,userID);
            return ResponseEntity.status(CREATED).body(new ApiResponse(user.getName() + " successfully updated!", user));
        } catch (EmailAlreadyExists e) {
            return ResponseEntity.status(400).body(new ApiResponse(e.getMessage(),null));
        }
    }
    ...
    ResponseEntity<ApiResponse> deleteUser(@RequestParam UUID userId){
        try {
            ...
            return ResponseEntity.status(200).body(new ApiResponse("Successfully deleted!", null));
        } catch (UserNotFoundException e) {
            return ResponseEntity.status(404).body(new ApiResponse(e.getMessage(),null));
        }
