...
public class OrderController {
    ...
    ResponseEntity<ApiResponse> placeOrder(@RequestParam UUID userId){
        try {
            ...
            return ResponseEntity.status(CREATED).body(new ApiResponse("Order successfully created!", order));
        } catch (Exception e) {
    ...
    ResponseEntity<ApiResponse> getOrderById(@PathVariable UUID orderId){
        ...
        try {
            ...
            return ResponseEntity.status(200).body(new ApiResponse("Success", order));
